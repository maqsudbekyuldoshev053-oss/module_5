...
class Advert(Base):
    ...
    view_owner: Mapped[int] = mapped_column(Integer, server_default='0')
...
class User(Base):
    ...
    advert: Mapped[list['Advert']] = relationship('Advert', back_populates='owner')
...
class District(Base):
    name: Mapped[str] = mapped_column(String(255))
    region_id: Mapped[int] = mapped_column(ForeignKey('regions.id'))
    region: Mapped[list['Region']] = relationship('Region', back_populates='districts')
    user: Mapped[list['User']] = relationship('User', back_populates='district')
...
with open('districts.csv', 'r') as f2,Session(engine) as session:
    district = csv.DictReader(f2)
    query = insert(District).values(list(district))
    session.execute(query)
    session.commit()
